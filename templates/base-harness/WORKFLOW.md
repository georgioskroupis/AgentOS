...
`lifecycle.mode: agent-owned` is the default source-faithful posture for
installed harnesses. The scheduler starts and observes isolated runs, while the
coding agent uses repo-local lifecycle tools for normal Linear progress:
run-start/running, substantive progress comments, PR metadata, handoff posting,
and the final `Human Review` transition.
...
tracker tools, an idempotency marker format with issue/run/attempt correlation,
allowed transitions, duplicate-comment behavior, and fallback behavior. Legacy
scheduler-owned lifecycle modes are disabled from public workflow configuration
and excluded from source-faithful certification.
...
10. Follow `lifecycle.mode: agent-owned`: use the `Run ID` from the AgentOS Run
   Context and the current attempt number when invoking repo-local lifecycle
   tools for run start, substantive comments, PR metadata, handoff posting, and
   `Human Review`.
...
- Use the repo-local lifecycle tools to post the handoff and move the issue to
  `Human Review` when the agent can act. Scheduler safety writes are reserved
  for no-agent-can-act failures.
