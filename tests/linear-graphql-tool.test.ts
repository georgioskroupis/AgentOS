...
describe("linear_graphql client tool", () => {
  it("advertises only for agent-owned Linear workflows with auth", () => {
    const agentOwned = fakeServiceConfig({ lifecycle: { ...fakeServiceConfig().lifecycle, mode: "agent-owned" } });
    ...
    expect(linearGraphqlClientTools(fakeServiceConfig({ lifecycle: { ...fakeServiceConfig().lifecycle, mode: "agent-owned" }, tracker: { ...fakeServiceConfig().tracker, apiKey: "" } }))).toEqual([]);
  });
  ...
  it("runs a valid query with variables and reports success", async () => {
    ...
    const result = await executeLinearGraphql(
      fakeServiceConfig({ lifecycle: { ...fakeServiceConfig().lifecycle, mode: "agent-owned" } }),
  ...
  it("accepts raw string shorthand", async () => {
    await expect(
      executeLinearGraphql(fakeServiceConfig({ lifecycle: { ...fakeServiceConfig().lifecycle, mode: "agent-owned" } }), "{ viewer { id } }", (async () => Response.json({ data: { viewer: { id: "me" } } })) as typeof fetch)
  ...
  it("rejects invalid input without transport", async () => {
    const config = fakeServiceConfig({ lifecycle: { ...fakeServiceConfig().lifecycle, mode: "agent-owned" } });
  ...
  it("returns structured availability and unsupported-tool failures", async () => {
    await expect(executeLinearGraphql(fakeServiceConfig({ lifecycle: { ...fakeServiceConfig().lifecycle, mode: "agent-owned" }, tracker: { ...fakeServiceConfig().tracker, apiKey: "" } }), "query Test { viewer { id } }")).resolves.toMatchObject({ success: false, error: { code: "missing_auth" } });
