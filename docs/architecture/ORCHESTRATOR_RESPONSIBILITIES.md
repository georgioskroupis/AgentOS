...
| Merge-state extension boundary | `src/merge-state-extension.ts`, `src/orchestrator.ts` | `source-core` | Core orchestration delegates merge-state processing through a typed extension and can run with the no-op implementation for source-core certification |
...
| Merge shepherding | `src/merge-state-shepherd-adapter.ts`, `src/orchestrator.ts`, `src/github.ts`, `src/landing-policy.ts` | `extension` | High-trust extension, not public-template default |
