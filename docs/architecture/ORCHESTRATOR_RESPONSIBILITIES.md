...
| Tracker polling and eligibility | `src/orchestrator.ts`, `src/orchestrator-tracker-guard.ts`, `src/tracker-boundaries.ts` | `source-core` | Core scheduler reads normalized tracker state before dispatch through `TrackerReader`; tracker adapters beyond Linear remain extensions |
...
| Post-validation extension boundary | `src/post-validation-extension.ts`, `src/orchestrator.ts` | `source-core` | Core completion can run the no-op post-validation implementation after validation and lifecycle evidence, while dogfood extension behavior plugs in separately |
| Merge-state extension boundary | `src/merge-state-extension.ts`, `src/orchestrator.ts` | `source-core` | Core orchestration delegates merge-state and approved-PR landing decisions through a typed extension and can run with the no-op implementation for source-core certification |
...
| Review and fixer loop | `src/orchestrator.ts`, `src/reviewer-runner.ts`, `src/review.ts`, `src/post-validation-review-adapter.ts` | `extension` | Source-aligned extension of the core loop, bounded by review policy and routed through the post-validation adapter |
...
| Merge shepherding | `src/merge-state-shepherd-adapter.ts`, `src/github.ts`, `src/landing-policy.ts` | `extension` | High-trust extension owns landing gates, draft-ready promotion, branch freshness, merge execution, cleanup, and Done transitions; not public-template default |
| Scope scoring and planning/decomposition helpers | `src/scope-report.ts`, `src/scope-report-scoring.ts`, `src/linear-planned-issue-types.ts`, `src/linear-planned-issues.ts`, `scripts/agent-linear-plan-issues.sh` | `extension` | Core may stop unsafe broad dispatch; issue creation and decomposition helpers remain optional planning behavior behind `PlanningIssueWriter` |
| Monitor read-only listener | `src/http-server.ts`, `dashboard/` | `extension` | Optional observability extension serving the lean monitor shell plus snapshot, SSE stream, and health routes; not a scheduler control plane |
