...
| Validation evidence and validation budget | `src/agent-owned-lifecycle-evidence.ts`, `src/validation-budget.ts`, `src/orchestrator.ts`, `src/issue-state.ts` | `source-core` | Handoff evidence must verify before review, and unchanged-head full validation reuse is explicit and profile-bound |
