...
| Dashboard/API state | `src/http-server.ts`, `dashboard/` | `extension` | Optional observability extension; not a scheduler control plane |
| Lean monitor snapshot and launcher | `src/monitor-extension-contracts.ts`, `docs/architecture/LEAN_MONITOR_CONTRACT.md`, `docs/architecture/MONITOR_DELETION_MANIFEST.md` | `extension` | Snapshot assembly, browser UI, standalone Mac launcher, and deletion of legacy dashboard/API touchpoints stay outside source-core orchestration |
