...
| Validation budget | `src/validation-budget.ts`, `src/orchestrator.ts`, `WORKFLOW.md` | Full-suite proof is budgeted per unchanged head and may be reused only when head, workflow hash, trust mode, automation profile, repair policy, and risk profile still match. |
