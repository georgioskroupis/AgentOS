...
| Tracker read boundary and Linear default | `src/tracker-boundaries.ts`, `src/tracker-adapters.ts`, `src/linear.ts`, `src/orchestrator-tracker-guard.ts` | Core orchestration may read normalized issue state and eligibility through a tracker boundary. Linear is the production default. Additional tracker adapters are extensions. |
...
| --- | --- | --- |
| Review and fixer loop | `src/reviewer-runner.ts`, `src/review.ts`, `src/orchestrator.ts` | Extension over core handoff. Blocking findings and fixer turns must not become a prerequisite for source-core dispatch or handoff verification. |
...
| CI diagnostics and repair | `src/ci-diagnostics.ts`, `src/ci-retry.ts`, `src/orchestrator-ci-retry.ts` | Mechanical repair extension for trustworthy logs and permitted PR/network access. Core can report failed validation without repairing CI. |
...
| Merge shepherd | `src/orchestrator.ts`, `src/github.ts`, `src/landing-policy.ts` | High-trust extension for `Merging` issues; not a public-template default and not required for core lifecycle certification. |
| Monitor placeholder listener | `src/http-server.ts`, `dashboard/` | Optional observability extension. It serves only the lean monitor shell and must not become a scheduler control plane. |
| Lean monitor snapshot and launcher | `src/monitor-extension-contracts.ts`, `docs/architecture/LEAN_MONITOR_CONTRACT.md`, `docs/architecture/MONITOR_DELETION_MANIFEST.md` | Optional observability extension over source-core monitor events. Snapshot assembly, UI sections, timing rows, top sinks, human action rendering, browser mode, and standalone Mac launcher behavior must not become source-core scheduler policy. |
...
| Planning and decomposition helpers | `src/linear-planned-issues.ts`, `src/scope-report-scoring.ts`, `scripts/agent-linear-plan-issues.sh` | Planning/decomposition output and child/follow-up issue creation are extensions. Core may stop broad unsafe dispatch, but it does not need to create decomposition issues. |
