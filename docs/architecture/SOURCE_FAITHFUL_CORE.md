...
| --- | --- | --- |
| Review and fixer loop | `src/reviewer-runner.ts`, `src/review.ts`, `src/orchestrator.ts`, `src/post-validation-review-adapter.ts` | Extension over core handoff. Blocking findings and fixer turns must not become a prerequisite for source-core dispatch or handoff verification; dogfood routes them through the post-validation adapter. |
